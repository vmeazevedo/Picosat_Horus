...
def live_stream():
    ...
    # Plot do sensor de tensão e temperatura interna do microcontrolador
    plt.subplot(4,2,8)
    plt.title("Live Streaming - Telemetria interna do Microcontrolador")
    plt.grid(True)
    #plt.ylabel('Tensão de IN')
    plt.plot(tensão_in, 'ro-', color='purple', label='VIN')
    plt.legend(loc='upper left')
    plt2=plt.twinx()
    plt2.plot(temperatura_int, 'bo-', color='orange', label='Temp. Int. °C')
...
while True:
    ...
    vin = float(dataArray[7])
    temp_int = float(dataArray[8])
    ...
    tensão_in.append(vin)
    temperatura_int.append(temp_int)
